...
#define WANT_POINTER(x) do { if( 0 == (x)) { fprintf(stderr, "\nTest %zu\n" #x " at line %d failed unexpectedly: %s\n", test_count, __LINE__, djpt_last_error()); abort(); } pthread_mutex_lock(&counter_lock); ++test_count; pthread_mutex_unlock(&counter_lock); } while(0)
  ...
  while(!done)
  {
    ...
    case 0:

      {
        ...
        WANT_SUCCESS(djpt_insert(db, tokens[row], tokens[col], tokens[token], tokenlen, DJPT_REPLACE));

        WANT_SUCCESS(djpt_get(db, tokens[row], tokens[col], &ret, &retsize));
    ...
    case 1:

      {
        ...
        {
          WANT_FAILURE(djpt_get(db, tokens[row], tokens[col], &ret, &retsize));
        }
      }
    ...
    case 2:

      {
        if(v->data)
        {
          WANT_SUCCESS(djpt_remove(db, tokens[row], tokens[col]));
        }
        else
        {
          WANT_FAILURE(djpt_remove(db, tokens[row], tokens[col]));
    ...
    case 3:

      {
        ...
        WANT_SUCCESS(djpt_insert(db, tokens[row], tokens[col], tokens[token], tokenlen, DJPT_APPEND));
      }
    ...
    case 4:

      {
        if(v->data)
        {
          int has_key = djpt_has_key(db, tokens[row], tokens[col]);

          if(0 == djpt_insert(db, tokens[row], tokens[col], tokens[token], tokenlen, 0))
          {
            fprintf(stderr, "djpt_insert of \"%s\", \"%s\" succeeded unexpectedly.  Has key: %d\n",
        ...
        else
        {
          ...
          WANT_SUCCESS(djpt_insert(db, tokens[row], tokens[col], tokens[token], tokenlen, 0));
        }
      }
    ...
    case 5:

      {
        ...
        if(vfy_count)
        {
          WANT_SUCCESS(djpt_column_scan(db, tokens[col], cell_callback, &count, 0));
          WANT_TRUE(count == vfy_count);
        }
        else
        {
          WANT_TRUE(0 == djpt_column_scan(db, tokens[col], cell_callback, &count, 0) || errno == ENOENT);
          WANT_TRUE(count == vfy_count);
        }
      }
    ...
    {
      WANT_SUCCESS(djpt_remove_column(db, tokens[col], 0));
      WANT_FAILURE(djpt_has_column(db, tokens[col]));
    ...
    {
      WANT_SUCCESS(djpt_compact(db));
    }
    else if((rand_r(&seed) % 30000) == 0)
    {
      WANT_SUCCESS(djpt_major_compact(db));
    }
